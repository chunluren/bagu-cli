...
### Q: 重新 import 同一文件会怎样？
A: SHA256 比对 — 内容没变就跳过。改了或带 `--force` 时触发 upsert：
- **题面没变** → 卡片 `id` + 复习历史保留（v1.1+），只更新答案 / 章节归属 / 时间戳
- **题面变了** → 视为新卡（旧的 stale 卡片删掉，CASCADE 清它的复习历史）
- **新增的题** → 新插
- **被删掉的题** → 跟着 CASCADE 清掉

> 题面 = 归一化后的 `**Q...** ` 内容（trim + 内部空白合并 + ASCII 小写）。
> 加 / 删空格、改大小写、换 TAB 都不影响匹配。
...
- [x] **稳定 key**（v1.1）：用 `sha256(topic_name + "::" + normalized_question)[:32]` 当稳定键，复习历史跨重新导入保留
